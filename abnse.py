...
class bse:
    ...
    def compute_moments(self):
        ...
        post_cov_F = self.post_cov_F
        w = self.w
        return w, post_mean, post_cov, post_mean_r, post_cov_r, post_mean_i, post_cov_i, post_cov_F
    ...
    def plot_interpolation(self, flag = None,loc = None,fig = None, ax = None):
        # posterior moments for time
        ax.plot(self.x, self.y, '.r', markersize=10, label='observations')
        error_bars = 2 * np.sqrt(np.diag(self.post_cov))
        time_window = np.linspace(self.tao-self.b/2,self.tao+self.b/2,1000)
        ax.fill_between(self.time, self.post_mean - error_bars, self.post_mean + error_bars, color= 'blue',
                          alpha=0.1, label='95% error bars')
        ax.plot(self.time , self.post_mean, color='blue', label='posterior mean')
        return fig, ax
    ...
    def plot_power_spectral_density_new(self, how_many, flag=None,n=None,fig = None,ax = None ):
        #posterior moments for frequency
        ...
        samples = np.zeros((freqs,how_many))
        for i in range(how_many):
            sample = np.random.multivariate_normal(self.post_mean_F,(self.post_cov_F+self.post_cov_F.T)/2 + 1e-2*np.eye(2*freqs))
            samples[:,i] = sample[0:freqs]**2 + sample[freqs:]**2
        ax.plot(self.w,samples, linewidth=0.1, color='red', alpha=0.3)
        if n == None:
            ax.plot(self.w,samples[:,0], color= 'red', linewidth=0.1,alpha=0.3, label='posterior samples of full signal')
        else:
            ax.plot(self.w,samples[:,0], color='red', linewidth=0.1 ,alpha=0.3, label='posterior samples of {}th segment'.format(n))
        posterior_mean_psd = self.post_mean_r**2 + self.post_mean_i**2 + np.diag(self.post_cov_r + self.post_cov_i)
        if n == None:
            ax.plot(self.w,posterior_mean_psd, color='black', linewidth=2.0,label = '(analytical) posterior mean of full signal')
        else:
            ax.plot(self.w,posterior_mean_psd, color= color_list[n], linewidth=2.0,label = '(analytical) posterior mean of {} th segment'.format(n))
        if flag == 'show peaks':
            peaks, _  = find_peaks(posterior_mean_psd, prominence=500000)
            widths = peak_widths(posterior_mean_psd, peaks, rel_height=0.5)
            ax.stem(self.w[peaks],posterior_mean_psd[peaks], markerfmt='ko', label='peaks')
        ...
        w = self.w
        posterior_mean_psd =posterior_mean_psd
        return w, posterior_mean_psd,ax,fig
...
def PSD(w,wq,vq,theta):
    return (wq/np.sqrt(8*np.pi*vq))* (np.exp(-(w-theta)**2/(2*vq))+np.exp(-(w+theta)**2/(2*vq)))
...
def freq_covariances(x, y, b, vq, theta, tao, wq=1, kernel='sm'):
    if kernel == 'sm':
        N = len(x)
        # compute kernels
        K =Spec_Mix_spectral_real(x, y, b, vq, theta, tao, wq) + Spec_Mix_spectral_real(x, y, b, vq,-theta, tao, wq)
        P = Spec_Mix_spectral_real(x, -y, b, vq, theta, tao, wq) + Spec_Mix_spectral_real(x, -y, b,vq, -theta, tao, wq)
